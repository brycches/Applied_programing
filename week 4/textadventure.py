...
class Character:
    ...
    def collides_with_walls(self, dx, dy, walls):
        ...
        next_y = self.y + dy
        for wall in walls:
            if wall.collidepoint(next_x, next_y) or wall.collidepoint(next_x + self.size, next_y) or \
               wall.collidepoint(next_x, next_y + self.size) or wall.collidepoint(next_x + self.size, next_y + self.size):
                return True
        return False
...
class Room:
    ...
    def generate_event(self):
        events = [
            ...
            ("You find an ancient scroll, do you read it?", "Read it", "Ignore it", "You learn a new skill.", "scroll"),
            ...
            ("You encounter a riddle, do you try to solve it?", "Solve it", "Ignore it", "You solve the riddle and gain a reward.", "riddle"),
            ("You find a staircase leading down, do you descend?", "Descend", "Stay", "You descend to a new level.", "staircase")
        ]
    ...
    def draw(self, surface):
        ...
        if self.has_event_object and not self.event_triggered:
            if self.event[4] == "goblin":
                ...
                pygame.draw.rect(surface, WHITE, self.event_rect)
            elif self.event[4] == "staircase":
                pygame.draw.rect(surface, GRAY, self.event_rect)

# Create a dungeon
def generate_dungeon(width, height):
    room_types = ["empty", "left_wall", "right_wall", "top_wall", "bottom_wall"]
    return [[Room(random.choice(room_types)) for _ in range(height)] for _ in range(width)]

dungeon_width, dungeon_height = 5, 5
dungeon = generate_dungeon(dungeon_width, dungeon_height)

# Starting room coordinates
current_room_x, current_room_y = 2, 2

# Create a character instance in the center of the room
...
def handle_event_choice(choice):
    global event_active, dungeon, current_room_x, current_room_y, player, display_text, current_room
